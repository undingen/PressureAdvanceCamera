...
class LineAnalyzer:
    ...
    def _find_contours(self):
        ...
        self.contours = [
            c for c in contours if cv2.contourArea(c) >= self.min_blob_area
        ]
    ...
    def get_smoothest_lines(self, top=5):
        ...
        sorted_lines = sorted(
            enumerate(self.lines), key=lambda x: x[1][3]
        )  # Sort by metric focusing on problematic areas
        return [
            (i + 1, line_data[2], line_data[3]) for i, line_data in sorted_lines[:top]
        ]
