...
class RetrieveRect:
    ...
    def _correct_perspective(self, image, corners):
        ...
        width_a = np.sqrt(((corners[0][0] - corners[1][0]) ** 2) + ((corners[0][1] - corners[1][1]) ** 2))
        width_b = np.sqrt(((corners[3][0] - corners[2][0]) ** 2) + ((corners[3][1] - corners[2][1]) ** 2))
        ...
        corrected_img = cv2.warpPerspective(image, perspective_matrix, (dst_width, dst_height))
